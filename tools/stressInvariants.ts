 ...
 * a board that is fast and wrong sends somebody to a pole for a bus that is not coming.
 *
 *   pnpm exec tsx tools/stressInvariants.ts
 *
 * Every stop, every ten minutes, on each kind of day, roughly 180,000 boards. The point
 * is the edges: first and last departure, the rollover past midnight, and the five-minute
 * window where a departure is overdue but still listed.
 */
  ...
  for (let minute = 0; minute < 24 * 60; minute += 10) {
    ...
    for (const stop of BUS_STOPS) {
      ...
      for (const a of arrivals) {
        ...
        if (!Number.isFinite(a.etaMinutes) || a.etaMinutes < 0) fail('a departure is a negative or non-finite number of minutes away', `${where}: ${a.etaMinutes}`);
        // The board looks two hours ahead and keeps five minutes of overdue behind it.
        if (a.etaMinutes > 120) fail('a departure is past the board’s own horizon', `${where}: ${a.etaMinutes} min`);
        if (a.overdueMinutes !== undefined && (a.overdueMinutes <= 0 || a.overdueMinutes > 5)) fail('an overdue departure is outside the five-minute window', `${where}: ${a.overdueMinutes} min`);
        if (a.overdueMinutes !== undefined && a.etaMinutes !== 0) fail('an overdue departure is not sitting at zero minutes', `${where}: eta ${a.etaMinutes}`);
        ...
        if (!stop.lines.includes(a.lineId)) fail('a stop lists a line its own record says does not call there', `${stop.name} ${a.lineId}`);
      }
    }
  }
}
...
for (const line of BUS_LINES) {
  for (const [i, direction] of line.directions.entries()) {
    ...
    if (direction.legSeconds.length !== direction.stops.length - 1) fail('a direction has the wrong number of legs', `${line.id}/${i}: ${direction.legSeconds.length} legs for ${direction.stops.length} stops`);
    if (new Set(direction.stops).size !== direction.stops.length) fail('a direction visits the same stop twice in one run', `${line.id}/${i}`);
  }
}
