 ...
 *
 * Four requests to other people's servers, so by hand or on the weekly schedule in
 ...
 * only test is whether the server's own parsers still find anything; the pole page has
 * departures in service hours and nothing outside them, and the check says which it is),
 * and the capped read against the real transport, which tools/test.ts can only prove locally.
 */
