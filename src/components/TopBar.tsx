  ...
  return (
    ...
    <header ref={boxRef} className="relative border-b border-line bg-bg px-3.5 py-3 lg:px-6">
      <div className="flex items-center gap-2">
        ...
        {/* The count of notices was only inside the drawer, so on a phone a retention
            existed for whoever happened to open the menu. The rail shows it always; so
            does this now. Same badge as the drawer row, so they read as one thing. */}
        <button
          onClick={onOpenMenu}
        ...
        >
          ...
          {alertCount > 0 && (
            <span
              className="anim-badge-in tnum absolute -right-1.5 -top-1.5 min-w-5 rounded-control bg-warn px-1 text-center text-label font-bold leading-5 text-warn-ink"
              aria-hidden="true"
            >
              {alertCount}
            </span>
          ...
          {stops.map((stop) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                {/* Whole, on two lines if it takes two: the rows were truncated, and on a
                    phone three poles of one street differed only in the part cut off. */}
